...
Auto-pricer + live dashboard + (optional) auto-trader for Kalshi SOL hourly
markets (`KXSOLD`). Polls Kalshi + Coinbase, prices each strike with a
...
- **Auto-trade** (`trade.py`, opt-in). A separate POST-allowed client +
  executor with hardcoded risk guards. **Read [TRADING.md](TRADING.md)
  before running this.**
...
`KXSOLD` markets settle against the simple average of **CF Benchmarks' Solana
Real-Time Index (SRTI)** over the final 60 seconds before close. The SRTI is a
basket across multiple exchanges (subject to CF revisions). This pricer uses
**Coinbase SOL-USD as an SRTI proxy** because real-time SRTI requires a CF
Benchmarks subscription. Coinbase tracks SRTI tightly under normal conditions
but can drift during fast moves or exchange-specific events. Treat any apparent
edge under a few dollars in SOL terms as potentially basis noise rather than
mispricing.
...
# Live dashboard (engine + UI in one process). Open http://127.0.0.1:8002
python dashboard.py
...
| `event_ticker`  | hourly event, e.g. `KXSOLD-26MAY0623`                  |
| `market_ticker` | strike-level ticker, e.g. `KXSOLD-26MAY0623-T179.99` |
| `strike`        | the strike (SRTI must be > strike for YES)             |
...
| `model_prob`    | P(SOL > strike at close) under zero-drift lognormal    |
...
| `proxy_source`  | `'coinbase'` (the SRTI proxy we used)                  |
...
sol-pricer/
  src/
    ...
    sol_feed.py        # Coinbase spot + 1m candles
