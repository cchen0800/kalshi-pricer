...
from src.sol_feed import CoinbaseFeed
...
def _event_title(event_ticker: str) -> str:
    ...
    `KXSOLD-26APR2802` → `Solana price tomorrow at 2am EDT`.
    ...
    return f"Solana price {when} at {time_str} {ny.strftime('%Z')}"
...
app = FastAPI(lifespan=lifespan, title="sol-pricer dashboard")
...
def api_spot() -> JSONResponse:
    """Live SOL spot from Coinbase, fetched fresh on every request.
...
if __name__ == "__main__":
    ...
    p.add_argument("--port", type=int, default=int(os.environ.get("DASHBOARD_PORT", "8002")))
    p.add_argument(
        "--no-engine",
        action="store_true",
