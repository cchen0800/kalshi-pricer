...
def make_row(
    *,
    market: str = "KXSOLD-26MAY0113-T80000",
    event: str = "KXSOLD-26MAY0113",
...
def test_notional_cap_caps_count(db, monkeypatch):
    ...
    row2 = make_row(market="KXSOLD-OTHER2", model_prob=0.80, yes_ask=0.50, yes_bid=0.45)
...
def test_rate_limit_blocks_after_n_orders(db):
    ...
    for i in range(MAX_ORDERS_PER_MINUTE):
        row = make_row(market=f"KXSOLD-RATE-T{i}", model_prob=0.80, yes_ask=0.10, yes_bid=0.05)
        d = ex.handle_poll([row])
        assert d.placed is True, f"order #{i} should have placed: {d.reason}"
    # 5th should hit the rate limit, regardless of edge.
    row = make_row(market="KXSOLD-RATE-T99", model_prob=0.80, yes_ask=0.10, yes_bid=0.05)
    d = ex.handle_poll([row])
    assert d.placed is False
...
def test_buy_no_uptrend_guard_blocks_weak_entries(db, monkeypatch):
    ...
    row = make_row(
        market="KXSOLD-26MAY0113-T92",
        event="KXSOLD-26MAY0113",
        strike=92.0,
        model_prob=0.20,
        yes_bid=0.20,
        yes_ask=0.25,
...
def test_buy_no_concentration_cap_keyed_by_no_side(db, monkeypatch):
    ...
    market = "KXSOLD-26MAY0113-T80000"
...
def test_buy_no_notional_cap_uses_aggregate_open_notional(db, monkeypatch):
    ...
    row2 = make_row(market="KXSOLD-OTHER-T1", model_prob=0.10, yes_bid=0.20,
...
def test_top_k_places_multiple_distinct_strikes(db, monkeypatch):
    ...
    ex = Executor(db, trader=None, live=False, profile=_legacy_profile())
    rows = [
        make_row(market="KXSOLD-A-T1", strike=80_500.0, model_prob=0.80,
                 yes_bid=0.20, yes_ask=0.25),
        make_row(market="KXSOLD-A-T2", strike=80_600.0, model_prob=0.80,
                 yes_bid=0.20, yes_ask=0.30),
        make_row(market="KXSOLD-A-T3", strike=80_700.0, model_prob=0.80,
                 yes_bid=0.20, yes_ask=0.35),
        make_row(market="KXSOLD-A-T4", strike=80_800.0, model_prob=0.80,
...
def test_top_k_running_notional_blocks_later_candidates(db, monkeypatch):
    ...
    ex = Executor(db, trader=None, live=False, profile=_legacy_profile())
    rows = [
        make_row(market=f"KXSOLD-NCAP-T{i}", strike=80_500.0 + i * 100,
...
def test_edge_proportional_sizing_scales_with_conviction(db, monkeypatch):
    ...
    row_low = make_row(market="KXSOLD-LO-T1", model_prob=0.35, yes_ask=0.25, yes_bid=0.20)
    ...
    row_high = make_row(market="KXSOLD-HI-T1", model_prob=0.80, yes_ask=0.25, yes_bid=0.20)
