...
`KXSOLD-26APR2623`), a strike ladder, and the settlement clause that mentions
CF Benchmarks SRTI. If you see a 4xx error, your auth is wrong — fix that
first.
...
|---|---|---|
| `Event` | The Kalshi event ticker. Format: `KXSOLD-YYMMMDDHH`. | Confirm you're looking at the right hour. |
| `SOL spot` | Coinbase SOL-USD last trade. | Eyeball reasonableness vs. the strike ladder. |
| `Realized σ` | Annualized vol from the last 60 1m closes. | Big jumps = regime change; trust the model less right after. |
| `Time to close` | Minutes until trading halts (SRTI averaging starts then). | Pricing has different character at T-1h vs T-3min. |
...
- **Strike** — SRTI must close *above* this number for YES to resolve YES.
...
- Later than 3 min: the SRTI 60-second average dominates, our spot-based
...
FROM polls
WHERE event_ticker = 'KXSOLD-26APR2623'
...
- **The 60-second SRTI average**: at T < 3 min, the right model is "average
  of 60 correlated 1-second SRTI samples," not "terminal price." The current
  pricer overstates volatility this close to settle.
- **Coinbase ≠ SRTI**: when funding rates blow out or one exchange has an
  outage, Coinbase can drift several dollars from SRTI. A "$10" edge at
...
- No CF Benchmarks subscription. Using Coinbase as SRTI proxy.
