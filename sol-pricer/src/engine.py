...
from src.sol_feed import CoinbaseFeed, closes, ohlc
...
SRTI_AVERAGING_WINDOW_SECONDS = 60.0  # Kalshi SRTI averages over the final 60s before close.
...
def build_poll_rows(
    *,
    ...
    averaging_window_seconds: float = SRTI_AVERAGING_WINDOW_SECONDS,
...
# Regime gates ported from BTC/ETH. SOL-specific tuning should be revisited
# once enough SOL settled events accumulate to bucket the same way.
# Starting with the same BTC gates as a conservative baseline.
#
...
def run_one_poll(
    *,
    ...
    # the SRTI averaging window. When we're inside the final 60s, this becomes
    ...
    if averagers is not None:
        ...
        if seconds_to_settlement < SRTI_AVERAGING_WINDOW_SECONDS:
            close_s = close_utc.timestamp()
            realized_partial_avg = avg.average(
                window_start_s=close_s - SRTI_AVERAGING_WINDOW_SECONDS,
...
def run(
    ...
    with KalshiClient() as kc, CoinbaseFeed() as feed, open_db(cfg.db_path) as db:
        ...
        while True:
            ...
            # Switch to fast cadence inside the SRTI averaging window neighborhood
