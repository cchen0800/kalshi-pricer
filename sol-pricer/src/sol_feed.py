"""Coinbase Exchange SOL-USD spot + 1m candles.

Public endpoints, no auth needed. We use Coinbase as a *proxy* for CF Benchmarks
SRTI (Kalshi's actual settlement reference for KXSOLD). See README for the basis caveat.

Spot fallback (used when Coinbase /ticker is 5xx) is a multi-source median
across Kraken and Gemini. Median across multiple sources degrades gracefully
if one source is slow or down.
"""
...
_FALLBACK_SOURCES: tuple[tuple[str, str], ...] = (
    ...
    ("gemini",   "https://api.gemini.com/v1/pubticker/solusd"),
)
_FALLBACK_TIMEOUT_S = 3.0  # per-source; tighter than primary so one slow source can't block the poll
...
class CoinbaseFeed:
    def __init__(
        ...
        # Single client for all fallback sources — full URLs are passed per call.
        # Connection pool keeps a warm connection per host across calls.
    ...
    def _record_spot_sample(self, spot: Spot) -> None:
        ...
        # 2h retention is enough for the engine's 60-bar vol window with slack.
        cutoff = ts_s - 7200
    ...
    def _get_spot_fallback(self) -> Spot:
        # Multi-source median across BRTI basket members. Hits each source
        # sequentially with a tight timeout so one slow source doesn't dominate
        # latency. Returns the median of all sources that respond; raises only
        ...
        if candles:
            ...
            if head_age > _CANDLES_STALENESS_THRESHOLD_S:
                ...
                if rebuilt:
                    ...
                    return rebuilt[-minutes:] if len(rebuilt) > minutes else rebuilt
                # Without enough fresh samples to rebuild, fall through and
                # use the stale candles. The engine's σ-clamp will bound the
                # damage; better than crashing.
                log.warning(
...
def _parse_kraken(d: dict) -> tuple[float, float, float]:
    ...
    v = next(iter(d["result"].values()))  # single-pair response
