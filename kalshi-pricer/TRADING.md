...
- The full strike ladder for the nearest open `KXBTCD` hourly event from Kalshi
  (typically 100–200 strikes spanning roughly ±2% around spot)
- Coinbase BTC-USD spot (proxy for the BRTI settlement index — see caveat below)
...
| `MIN_EDGE_CENTS` | `15.0` | Minimum actionable edge to act. Must clear: Kalshi taker fee (~2¢), spread (1–2¢), and BRTI/Coinbase basis noise (~5–10¢ in BTC terms). |
| `MAX_ORDERS_PER_MINUTE` | `4` | Rate limit on ourselves. Catches runaway loops; avoids tripping Kalshi's own rate limits. |
| `MIN_MINUTES_TO_CLOSE` | `5.0` | Stop trading 5 min before settle. Reason: the BRTI averages over the final 60s, and Coinbase can drift several dollars from BRTI in that window — the model is least reliable then. |
...
`KXBTCD` settles against **CF Benchmarks BRTI** averaged over the final 60s.
The BRTI is a basket of Coinbase, Bitstamp, Kraken, LMAX, Gemini, itBit. We
proxy with **Coinbase BTC-USD** because real-time BRTI requires a paid
subscription. Coinbase tracks BRTI tightly under normal conditions but can
