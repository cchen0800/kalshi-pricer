...
Launch:
    python dashboard.py                    # http://localhost:8001, owns its own engine
...
from src.eth_feed import CoinbaseFeed
...
def _event_title(event_ticker: str) -> str:
    ...
    `KXETHD-26APR2802` → `Ethereum price tomorrow at 2am EDT`.
    ...
    return f"Ethereum price {when} at {time_str} {ny.strftime('%Z')}"
...
app = FastAPI(lifespan=lifespan, title="eth-pricer dashboard")
...
def api_spot() -> JSONResponse:
    """Live ETH spot from Coinbase, fetched fresh on every request.
...
def api_trades(limit: int = 50, bot: str = "selective") -> JSONResponse:
    """Trade history + aggregate P&L for one of the asset's bots.
    ...
    with open_db(db_path) as db:
        ...
        s = summarize(db, trader, mode="live")
    profile = BOT_PROFILES.get(bot)
    if profile is not None:
        s["allocated_capital_usd"] = profile.max_notional_usd
...
if __name__ == "__main__":
    ...
    p.add_argument("--port", type=int, default=int(os.environ.get("DASHBOARD_PORT", "8001")))
    p.add_argument(
        "--no-engine",
        action="store_true",
