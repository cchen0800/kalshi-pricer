...
Model: S_T = S_0 * exp((-0.5 * sigma^2) * T + sigma * sqrt(T) * Z),  Z ~ N(0, 1)
       (drift = 0; appropriate for short-dated ETH with no carry)
...
def prob_above_strike_path_dependent(
    *,
    ...
    """P(ERTI time-average over final W seconds > strike).

    Kalshi KXETHD settles on the average of the CME CF Ether Reference Rate
    ...
    leading order at ETH/short-window scales (σ·√(60s) ≈ 0.13% on $4k).
