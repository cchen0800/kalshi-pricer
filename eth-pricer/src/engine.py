...
from src.eth_feed import CoinbaseFeed, closes, ohlc
...
ERTI_AVERAGING_WINDOW_SECONDS = 60.0  # Kalshi ERTI averages over the final 60s before close.
...
TICKER_RE = re.compile(r"^KXETHD-(\d{2})([A-Z]{3})(\d{2})(\d{2})$")
...
def build_poll_rows(
    *,
    ...
    averaging_window_seconds: float = ERTI_AVERAGING_WINDOW_SECONDS,
...
# the BTC dataset shows BUY_NO is +12.8¢/trade across σ<0.50 vs +7.3¢/trade
# at σ≥0.50. Applying the BUY_YES floor to BUY_NO filters out the band
# where the alpha lives. The 0.20 floor is a "calm-market garbage" cutoff —
# well below the band where BUY_NO has shown positive EV — and is
# intentionally separate from the BUY_YES floor so it can move
# independently. ETH-specific tuning should be revisited once enough ETH
# settled events accumulate to bucket the same way.
#
...
def run_one_poll(
    *,
    ...
    if averagers is not None:
        ...
        if seconds_to_settlement < ERTI_AVERAGING_WINDOW_SECONDS:
            close_s = close_utc.timestamp()
            realized_partial_avg = avg.average(
                window_start_s=close_s - ERTI_AVERAGING_WINDOW_SECONDS,
...
def run(
    ...
    with KalshiClient() as kc, CoinbaseFeed() as feed, open_db(cfg.db_path) as db:
        ...
        while True:
            ...
            # Switch to fast cadence inside the ERTI averaging window neighborhood
