...
MAX_CONTRACTS_PER_EVENT = 20          # same-side cap across the strike ladder of one event.
                                      ...
                                      # is one bet, not N.
MAX_OPEN_STRIKES_PER_EVENT_SIDE = 2   # number of distinct same-side markets open per event.
...
BOT_PROFILES: dict[str, BotProfile] = {
    "selective": BotProfile(
        ...
        # 2026-05-14 defensive reset: the larger ETH size amplified a shared
        # BUY_NO/up-tape loss. Keep capital share elevated only after the
        # trend guard has fresh live data.
...
class Executor:
    ...
    def _candidate_sort_key(self, row: PollRow, side: str, edge: float) -> tuple[int, float]:
        """Closing sells first by edge, then buys by ROI rather than raw cents."""
        if side.startswith("SELL_"):
            return (0, -edge)
        limit_cents = self._limit_price_cents(row, side)
