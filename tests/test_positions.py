...
def test_in_flight_overlay_expires_after_window(db):
    ...
    insert_intent(db, market="KXBTCD-OLD-T1", count=5, limit_cents=25, ts_ms=old_ts)
    snap = snapshot(db, FakeTrader())
    assert snap is not None
    assert snap.open_notional_usd == 0.0
...
def test_settled_loser_attributes_only_bot_cost(db):
    ...
    insert_intent(db, market=market, action="buy", count=5, limit_cents=25,
                  kalshi_order_id=fill["order_id"])
    insert_intent(db, market="KXBTCD-Y-T2", action="buy", count=4, limit_cents=30)
    settlements = [{
        "ticker": market,
        "settled_time": now_iso(),
        "market_result": "no",
...
def test_pending_intent_counts_as_in_flight_buy(db):
    ...
    insert_intent(db, market="KXBTCD-P-T1", count=5, limit_cents=20, status="pending")
    snap = snapshot(db, FakeTrader())
    assert snap is not None
...
def test_coid_prefix_attribution_recovers_unlinked_fill(db):
    ...
    fill = make_fill(
        ...
        client_order_id="btcp-abc123", order_id="external-order-id",
    )
    snap = snapshot(db, FakeTrader(fills=[fill]), bot_coid_prefix="btcp-")
    assert snap is not None
...
def test_sibling_bot_fill_not_attributed(db):
    ...
    KXBTCD market our bot also trades."""
    market = "KXBTCD-SHARED-T1"
    sibling_fill = make_fill(
        market=market, action="buy", count=5, fill_cents=30,
        client_order_id="btca-other", order_id="sibling-order",
    )
    snap = snapshot(db, FakeTrader(fills=[sibling_fill]), bot_coid_prefix="btcp-")
    assert snap is not None
    assert snap.open_notional_usd == 0.0
    assert snap.open_contracts_by_market == {}
